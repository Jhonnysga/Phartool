...
def create_icon_png():
    """Genera un icono PNG de 192x192 (color turquesa) con texto 'PT'."""
...
def create_logo_xml():
    """Crea un drawable XML vectorial con un cuadrado turquesa."""
...
def create_project():
    ...
    # Copiar el icono a otras densidades
...
public class ControlEtiquetadoActivity extends AppCompatActivity {
    ...
    private void iniciarEscaneo() {
        ...
        }).addOnFailureListener(e -> Toast.makeText(this, "Error escaneo", Toast.LENGTH_SHORT).show());
    }
    ...
    private void imprimirEtiqueta(Producto p) {
        ...
        printerService.print(device, tspl, new BluetoothPrinterService.Callback() {
            @Override public void onSuccess() {
                runOnUiThread(() -> { Toast.makeText(ControlEtiquetadoActivity.this, "🏷️ Etiqueta impresa", Toast.LENGTH_SHORT).show();
                    tvEstado.setText("Etiqueta impresa. Escanea siguiente."); limpiarPantalla(); });
            }
            @Override public void onError(String error) { runOnUiThread(() -> tvEstado.setText("❌ Error: " + error)); }
        });
    }
    ...
    private void generarCodigoBarras(Producto p) {
        ...
        printerService.print(device, tspl, new BluetoothPrinterService.Callback() {
            @Override public void onSuccess() {
                runOnUiThread(() -> { Toast.makeText(ControlEtiquetadoActivity.this, "📦 Código de barras impreso", Toast.LENGTH_SHORT).show();
                    tvEstado.setText("Código de barras impreso. Escanea siguiente."); limpiarPantalla(); });
            }
            @Override public void onError(String error) { runOnUiThread(() -> tvEstado.setText("❌ Error: " + error)); }
        });
    }
...
public class EtiquetadoDirectoActivity extends AppCompatActivity {
    ...
    private static final int MAX_INTENTOS_FALLIDOS = 2, TIMEOUT_ESCANEO = 15000;
    private static final int PAUSA_POST_ESCANEO = 500, PAUSA_POST_IMPRESION = 2000;
    ...
    private void iniciarEscaneoConTimeout() {
        ...
        isScanning = true;
        timeoutRunnable = () -> {
            if (isScanning) { isScanning = false; tvEstado.setText("⏰ Tiempo agotado");
                Toast.makeText(this, "No se detectó escaneo. Volviendo al menú.", Toast.LENGTH_SHORT).show(); finish(); }
        };
        ...
        scanner.startScan()
            .addOnSuccessListener(barcode -> {
                ...
                isScanning = false;
                handler.postDelayed(() -> procesarCodigo(barcode.getRawValue()), PAUSA_POST_ESCANEO);
            })
        ...
        printerService.print(adapter.getRemoteDevice(mac), tspl, new BluetoothPrinterService.Callback() {
            @Override public void onSuccess() {
                runOnUiThread(() -> {
                    ...
                    handler.postDelayed(() -> { if (!isFinishing()) iniciarCicloEscaneo(); }, PAUSA_POST_IMPRESION);
                });
            }
...
public class ConfiguracionActivity extends AppCompatActivity {
    ...
    private void cargarSedesDesdeAPI() {
        ...
        progressDialog.setTitle("Cargando sedes");
        progressDialog.setMessage("Consultando lista de sedes...");
        progressDialog.setCancelable(false);
        progressDialog.show();
        new Thread(() -> {
            try {
                sedes = new SedeApiClient().obtenerSedes();
                runOnUiThread(() -> {
                    progressDialog.dismiss();
                    if (sedes.isEmpty()) { Toast.makeText(this, "No se encontraron sedes", Toast.LENGTH_SHORT).show(); return; }
    ...
    private void cargarSedesRespaldo() {
        sedes.clear();
        String[][] hardcoded = {{"CATEDRAL","Barquisimeto","02","1"},{"ESTE","Barquisimeto","03","1"},
            {"OESTE","Barquisimeto","04","1"},{"SAN BENITO (JEBE)","Barquisimeto","08","1"},
            {"LA FUNDACION","Barquisimeto","PRIN","3"},{"FARMACIA LA 21","Barquisimeto","09","1"},
            {"FARMACIA CERRITOS BLANCOS","Barquisimeto","11","1"},{"CLINIFARMA CABUDARE","Cabudare","01","1"},
            {"CHUCHO BRICEÑO","Cabudare","06","1"},{"LA MONTAÑITA","Cabudare","07","1"},
            {"PLAZA SAN PEDRO","Valera","13","1"},{"YARITAGUA","Yaritagua","PRIN","2"}};
...
""")
    ...
    # ===== GRADLE-WRAPPER.JAR (descargar desde internet) =====
    # En GitHub Actions, esto no es necesario porque se genera con `gradle wrapper`
    # Pero lo dejamos como placeholder para que el script lo cree si se ejecuta localmente.
    wrapper_jar_path = os.path.join(project_dir, "gradle/wrapper/gradle-wrapper.jar")
    if not os.path.exists(wrapper_jar_path):
        print("  ⚠️ gradle-wrapper.jar no encontrado. Se generará automáticamente al compilar.")
        # No lo creamos, porque el workflow lo genera con `gradle wrapper`.
